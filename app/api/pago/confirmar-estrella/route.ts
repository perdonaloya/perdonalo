...
export async function GET(req: NextRequest) {
  ...
  const tbkOrderId = searchParams.get("TBK_ORDER_ID");
...
async function handleCancelado(req: NextRequest, tbkToken: string, tbkOrderId: string) {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000";
  const ip =
  ...
  await registrarLog({
    ...
    tipo: "pago_estrella",
    evento: "cancelado",
    mensaje: `Pago cancelado por usuario — estrella ${estrella_id} — TBK_ORDER_ID: ${tbkOrderId}`,
          ...
          html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#0a0a2e;color:#fff;">
              ...
              <div style="background:#111132;border-radius:12px;padding:16px 20px;margin-bottom:16px;">
                ...
                <p style="color:#aaa;font-size:13px;margin:0;">ID producto: <strong style="color:#fff;">${estrella_id}</strong></p>
              </div>
